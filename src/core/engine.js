...
class Engine {
  ...
  async chat(userMessage) {
    ...
    // Trim history to last 40 messages (20 turns) to avoid token overflow
  ...
  async rawChat(prompt, maxTokens = 1024) {
    const cfg = config.load();
    if (!cfg.apiKey) throw new Error('no API key set');
    return await this._request(null, [{ role: 'user', content: prompt }], cfg, 3, maxTokens);
  }
  ...
  async _request(system, messages, cfg, retries = 3, maxTokens = 2048) {
    for (let i = 0; i < retries; i++) {
      try {
        if (this._isAnthropic(cfg)) return await this._anthropic(system, messages, cfg, maxTokens);
        return await this._openai(system, messages, cfg, maxTokens);
      } catch (e) {
  ...
  async _openai(system, messages, cfg, maxTokens = 2048) {
    const msgs = system ? [{ role: 'system', content: system }, ...messages] : messages;
    const res  = await fetch(`${cfg.baseUrl}/chat/completions`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${cfg.apiKey}` },
      body:    JSON.stringify({ model: cfg.model, messages: msgs, max_tokens: maxTokens }),
    });
    if (!res.ok) {
  ...
  async _anthropic(system, messages, cfg, maxTokens = 2048) {
    const body = { model: cfg.model || 'claude-sonnet-4-6', max_tokens: maxTokens, messages };
